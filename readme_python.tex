...
\texttt{sylow\_solver/} & Solver package: facts/disjunctions, substitution helper, theorem base classes (with precompiled arg patterns), proof search engine, default theorems, predicates, and config/logging helpers.\end{tabular}
...
\subsection{Substitutions and Matching}
Unification and theorem matching use a lightweight \texttt{Substitution} helper (a thin wrapper around the binding map) with convenience constructors and copy operations. Theorem premises precompile their argument patterns so matching avoids repeated prefix checks. Fact lookups are indexed by (predicate, arity) to reduce scanning.
