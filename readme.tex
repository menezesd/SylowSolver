\documentclass[11pt,
oneside]{article} % use "amsart" instead of "article" for AMSLaTeX format
...
\title{Automating Sylow's Theorems: A Framework Overview}
\author{Dean Menezes}
...
\begin{abstract}
This document presents a conceptual overview of a framework for automating proofs based on Sylow's Theorems in finite group theory. The framework employs a hybrid approach, combining a predefined set of group-theoretic techniques with a simple theorem-proving engine that searches for contradictions. By abstracting facts and theorems, the system can systematically explore properties of finite groups, particularly in demonstrating the non-existence of simple groups for various orders. Implementations of this framework exist in both Python and Haskell, offering practical applications of the described methodology. This paper details the core techniques, the abstraction model, and the underlying proof search strategy, alongside discussions on limitations and future directions.
\end{abstract}

This document provides a conceptual overview of the framework and theoretical techniques employed in an automated theorem prover designed to demonstrate properties of finite groups, particularly focusing on Sylow-style arguments. Concrete implementations of this framework exist in both Python and Haskell.

For detailed documentation, including module structure, setup instructions, and language-specific features of the Python implementation, please refer to \href{readme_python.tex}{readme\_python.tex}.

For similar detailed documentation of the Haskell implementation, please refer to \href{readme_haskell.tex}{readme\_haskell.tex}.
...
The theoretical framework described in this document has been concretely realized in two distinct programming language implementations: Python and Haskell. While both adhere to the core principles outlined here, they differ in their specific syntax, module organization, and language-specific features.

\subsection{Python Implementation}
The Python implementation serves as a direct realization of the theorem-proving concepts, particularly emphasizing the dynamic nature of fact and theorem manipulation. It is documented in detail in \href{readme_python.tex}{readme\_python.tex}.

\subsection{Haskell Implementation}
The Haskell implementation is a port of this framework, leveraging Haskell's strong type system and functional paradigms to provide a robust and extensible theorem prover. Its detailed documentation, including module structure and usage, can be found in \href{readme_haskell.tex}{readme\_haskell.tex}.
...
$100.$ %ACTUAL NUMBER? TABLE of group orders/tricks
...
(which is a string), followed by an (ordered) list of arguments. These
arguments can represent various entities, such as concrete symbols,
variables, or numeric literals. For example, the fact,
\texttt{(group, [G])} expresses the logical statement ``$G$ is a group",
while \texttt{(order, [G,n])} expresses ``the order of G is $n$".
The type and handling of arguments (e.g., whether numeric values are
treated as distinct from string representations) vary by implementation.
We note that the logical description associated with a fact is primarily
for human understanding and is not directly interpreted by the core
...
output by a theorem may contain an argument that acts as a placeholder
for a new, yet-to-be-named mathematical object. This allows theorems
to assert the existence of new entities without requiring them to
pre-allocate unique identifiers. The task of assigning a globally
unique name is delegated to the proof environment. For example, a
theorem might express "there exists a group of order $n$", where the
group's name is initially a placeholder. The specific syntax for such
placeholders varies between implementations; refer to the respective
language documentation for details.

Similarly, our framework supports arguments in templates that require
an exact literal match, preventing them from being treated as
variables that can be unified with other symbols. This is useful when
a specific numerical value or constant is critical for the theorem's
application. The syntax for denoting such exact matches is
implementation-dependent; consult the specific language documentation.
...
\section{Future Work}
This proof-of-concept work opens several avenues for future research and development:
\begin{itemize}
    \item \textbf{Expanding the Theorem Set:} Integrate more advanced group-theoretic techniques and theorems to broaden the range of group orders and properties that can be analyzed automatically. This could include theorems related to soluble groups, commutator subgroups, or specific group actions.
    \item \textbf{Improving Search Strategies:} Implement more sophisticated proof search algorithms beyond brute-force, such as heuristic search, goal-directed reasoning, or integration with SMT solvers, to handle more complex scenarios and mitigate combinatorial explosion, especially with disjunctions.
    \item \textbf{Interactive Proof Assistance:} Develop a user interface that allows human users to guide the prover, inspect partial proofs, or suggest new facts and theorems, creating a hybrid human-computer proof-assistant system.
    \item \textbf{Formal Verification and Correctness:} Explore methods to formally verify the correctness of the implemented theorems and the proof engine itself, potentially using proof assistants like Coq or Isabelle/HOL.
    \item \textbf{Generalization to Other Algebraic Structures:} Adapt the framework to automate proofs in other areas of abstract algebra (e.g., ring theory, field theory) or even other mathematical domains, by defining new fact types and theorem rules.
    \item \textbf{Performance Optimization:} Investigate performance bottlenecks in both Python and Haskell implementations and apply optimizations, including parallelization strategies for exploring disjunction branches.
\end{itemize}
